...
void odometry::estimate(SensorData *rawdata) {
  ...
  if (m_current_t_read - m_prev_t_read > 500000) {
    RCLCPP_WARN_STREAM(rclcpp::get_logger("odometry"),
                       "Detected a significant time gap: "
                           << "Current time: " << m_current_t_read << ", "
                           << "Previous time: " << m_prev_t_read);
  }
  m_prev_t_read = m_current_t_read;
  if (m_if_record) {
    if (m_record_file.is_open()) {
...
void odometry::init() {
  ...
  auto node = std::make_shared<rclcpp::Node>("record");
  m_record_file_path = "/home/segwayrmp_record";
  m_if_record = node->declare_parameter<bool>("if_record", m_if_record);
  m_record_file_path = node->get_parameter<std::string>("record_file_path", m_record_file_path);
...
Odometry odometry::GetOdometry() { return m_odometry_; }
